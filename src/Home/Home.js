...
const Home = () => {
    ...
    let data = [
      ...
      const markers = data.map(incident => ({
        position: [incident.Latitude, incident.Longitude],
        popupText: `${incident.title} - ${incident.location}`,
        count: 1 // You can set count based on some criteria if needed
      }));
    
      const handleSeeDetails = (incident) => {
        navigate(`/incident/${incident.id}`, { state: { incident } });
      };
                  ...
                  <div className="c-card">
                    <div className="c-card__content">
                      <header className="c-card__header">
                        <h2 className="c-card__title" data-title={slide.title}>{slide.title}</h2>
                      </header>
                      <img
                      ...
                      <footer className="c-card__footer">
                        <button 
                          onClick={() => handleSeeDetails(slide)} 
