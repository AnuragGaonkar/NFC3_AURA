...
import './Map.css';
import redpin from './pin.png';

const Map = ({ center, markers }) => {
      const redIcon = new L.Icon({
        iconUrl: 'C:/Users/ThDarKSouL/Desktop/crime-report/src/Map/pin.png', // Replace with your red icon URL
        iconSize: [32, 32],
        iconAnchor: [16, 32],
        popupAnchor: [0, -32]
    });
    const blueIcon = new L.Icon({
      iconUrl: redpin, // Replace with your blue icon URL
      iconSize: [32, 32],
      iconAnchor: [16, 32],
      popupAnchor: [0, -32]
    });
      ...
      return (
        ...
        >
            ...
            />
            {/* Static Mumbai marker with red icon */}
            <Marker position={defaultPosition} icon={redIcon}>
                <Popup>
                    Mumbai
                </Popup>
            </Marker>
            ...
            {/* Dynamic markers and circles from dataset */}
            {data.map((item) => (
                <React.Fragment key={item.id}>
                    <Marker position={[item.Latitude, item.Longitude]} icon={blueIcon}>
                        <Popup>
                    ...
                    <Circle
                        ...
                        radius={item.accuracyRadius || 300} // Default radius if not specified
                        color='blue'
                        fillColor='blue'
                        fillOpacity={0.4}
                    >
